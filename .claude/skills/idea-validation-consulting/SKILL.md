...
`memory/` 내 context 파일들을 Read 도구로 읽어 컨설팅 대상의 핵심 맥락을 파악합니다.
이후 단계에서 사용자에게 이미 파악된 내용을 중복 질문하지 않습니다.

> **Write-back 의무**: 작업 완료 후 메모리 업데이트 시, context 파일의 범위에 해당하는
> 새롭게 확인/검증된 사실이 있으면 해당 context 파일에 반영합니다.
> 확인된 사실만 기록하고, 작업별 산출물은 기록하지 않습니다.
